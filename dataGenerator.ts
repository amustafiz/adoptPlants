...
import { postAd } from "./pages/api/ads";
import { Cycle, Water, Sunlight } from "./types";

export async function generateFakeAdds(quantity = 10) {
  const startIndex = Math.floor(
    Math.random() * Object.values(seedData).length - quantity
  );

  const slicedSeedData = seedData.slice(startIndex, startIndex + quantity);

  const documentsArray = slicedSeedData.map((element, _) => {
    const {
      cycle,
      watering,
      sunlight,
      common_name: name,
      default_image: { small_url: imageUrl },
    } = element;
    return {
      cycle:
        Object.values(Cycle)[
          Math.floor(Math.random() * Object.values(Cycle).length)
        ],
      watering:
        Object.values(Water)[
          Math.floor(Math.random() * Object.values(Water).length)
        ],
      sunlight:
        Object.values(Sunlight)[
          Math.floor(Math.random() * Object.values(Sunlight).length)
        ],
      title: name,
      authorName: faker.name.firstName(),
      imageUrl,
      edible: Math.random() >= 0.5,
      poisonous: Math.random() >= 0.5,
      indoor: Math.random() >= 0.5,
      authorId: +faker.random.numeric(1),
      description: faker.lorem.lines(1),
      email: faker.internet.email(),
    };
  });

  for (const element of documentsArray) {
    console.log(element.title);
    await postAd(element);
  }
